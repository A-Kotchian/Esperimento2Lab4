...
freq = 10000.0                                #frequenza di campionamento S/[s]
...
waitT=tempo-float(samples)/float(freq)-tempo/100.0 #tempo da aspettare[s]

step=90.0/N_data                             #salto di Volt[V]
...
with open("risultato.csv","w") as f:
    ...
    with nidaqmx.Task() as task:
        ...
        for i in range(N_data):
            ...
            value+=step
            print(value)
            inst.write(("VOLT %d" % (100-value)))
            time.sleep(tempo/100.0)
            try:
            ...
            #datax.append(a) per plottare in funzione del voltaggio
            datay.append(mean)
            f.write("\n{0}, {1}".format(a ,mean))
            time.sleep(waitT)
